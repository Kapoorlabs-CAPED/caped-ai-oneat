...
class PlotSilverHistory(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.plot:
         ...
         #clear_output(True)
        idx = random.randint(1,self.X.shape[0] - 1)
        Printsilverpredict(idx,self.Trainingmodel, self.X, self.Y, self.key_categories, self.key_cord, self.gridx, self.gridy, plot = self.plot, nboxes = self.nboxes)
        
def Printsilverpredict(idx, model, data, Truelabel, key_categories,key_cord, gridx, gridy, plot = False, nboxes = 1):
...
def PrintVolumepredict(idx, model, data, Truelabel, key_categories,key_cord, gridx, gridy, gridz,  plot = False, nboxes = 1):
    ...
    for i in range(0, prediction.shape[0]):
        ...
        try:
              ...
              maxevent = np.argmax(prediction[i,:,:,:,:,:len(key_categories)], axis = -1)
              trueevent = np.argmax(Truelabel[0,0,0,0,:len(key_categories)], axis = -1)
        for (k,v) in key_categories.items(): 
           if v == maxevent: 
               maxlabel =  k
           if v == trueevent:
        ...
        for (k,v) in key_cord.items():
            try:
                   ...
                   print(k, prediction[i,:,:,:,:,len(key_categories) + v])
                   print('True positional value', k, Truelabel[0,0,0,0,len(key_categories) + v])
