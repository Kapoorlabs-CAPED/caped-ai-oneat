...
def extract_ground_event_truth(
    y_true, categories, grid_h, grid_w, grid_t, nboxes, box_vector
):

    true_box_class = y_true[..., 0:categories]

    true_nboxes = K.reshape(
        y_true[..., categories:],
        (-1, grid_h * grid_w * grid_t, nboxes, box_vector),
    )
...
def extract_ground_event_pred(
    y_pred, categories, grid_h, grid_w, grid_t, event_grid, nboxes, box_vector
):

    pred_box_class = y_pred[..., 0:categories]

    pred_nboxes = K.reshape(
        y_pred[..., categories:],
        (-1, grid_h * grid_w * grid_t, nboxes, box_vector),
    )
...
def extract_ground_event_volume_truth(
    y_true, categories, grid_h, grid_w, grid_d, nboxes, box_vector
):
    ...
    true_nboxes = K.reshape(
        y_true[..., categories:],
        (-1, grid_h * grid_w * grid_d, nboxes, box_vector),
    )
...
def extract_ground_event_volume_pred(
    y_pred, categories, grid_h, grid_w, grid_d, event_grid, nboxes, box_vector
):
...
def extract_ground_cell_pred(
    y_pred, categories, grid_h, grid_w, cell_grid, nboxes, box_vector
):

    pred_box_class = y_pred[..., 0:categories]

    pred_nboxes = K.reshape(
        y_pred[..., categories:], (-1, grid_h * grid_w, nboxes, box_vector)
    )
...
def extract_ground_cell_truth(
    y_truth, categories, grid_h, grid_w, nboxes, box_vector
):

    true_box_class = y_truth[..., 0:categories]

    true_nboxes = K.reshape(
        y_truth[..., categories:], (-1, grid_h * grid_w, nboxes, box_vector)
    )
...
def extract_ground_cell_pred_segfree(
    y_pred, categories, grid_h, grid_w, cell_grid, nboxes, box_vector
):
...
def extract_ground_cell_truth_segfree(
    y_truth, categories, grid_h, grid_w, nboxes, box_vector
):
...
def compute_conf_loss(
    ...
    intersect_wh = K.maximum(
        K.zeros_like(pred_box_wh),
        (pred_box_wh + true_box_wh) / 2
        - K.square(pred_box_xy[..., 0:1] - true_box_xy[..., 0:1]),
    )
    ...
    loss_conf = K.sum(
        K.square(true_box_conf * best_ious - pred_box_conf), axis=-1
    )
...
def compute_conf_loss_volume(
    ...
    # compute the intersection of all boxes at once (the IOU)
    intersect_whd = K.maximum(
        K.zeros_like(pred_box_whd),
        (pred_box_whd + true_box_whd) / 2
        - K.square(pred_box_xyz[..., 0:3] - true_box_xyz[..., 0:3]),
    )
    intersect_area = (
        intersect_whd[..., 0] * intersect_whd[..., 1] * intersect_whd[..., 2]
    )
    true_area = (
        true_box_whd[..., 0] * true_box_whd[..., 1] * true_box_whd[..., 2]
    )
    pred_area = (
        pred_box_whd[..., 0] * pred_box_whd[..., 1] * pred_box_whd[..., 2]
    )
    union_area = pred_area + true_area - intersect_area
    iou = tf.truediv(intersect_area, union_area)
...
def compute_conf_loss_static(
    ...
    intersect_wh = K.maximum(
        K.zeros_like(pred_box_wh),
        (pred_box_wh + true_box_wh) / 2 - K.square(pred_box_xy - true_box_xy),
    )
    intersect_area = intersect_wh[..., 0] * intersect_wh[..., 1]
    true_area = true_box_wh[..., 0] * true_box_wh[..., 1]
    pred_area = pred_box_wh[..., 0] * pred_box_wh[..., 1]
...
def calc_loss_xywh(true_box_xy, pred_box_xy, true_box_wh, pred_box_wh):
    ...
    loss_xy = K.sum(
        K.sum(K.square(true_box_xy - pred_box_xy), axis=-1), axis=-1
    )
    loss_wh = K.sum(
        K.sum(K.square(K.sqrt(true_box_wh) - K.sqrt(pred_box_wh)), axis=-1),
        axis=-1,
    )
...
def calc_loss_xyzwhd(true_box_xyz, pred_box_xyz, true_box_whd, pred_box_whd):
    ...
    loss_xyz = K.sum(
        K.sum(K.square(true_box_xyz - pred_box_xyz), axis=-1), axis=-1
    )
    loss_whd = K.sum(
        K.sum(K.square(K.sqrt(true_box_whd) - K.sqrt(pred_box_whd)), axis=-1),
        axis=-1,
    )
...
def calc_loss_angle(true_box_angle, pred_box_angle):
    ...
    loss_angle = K.sum(
        K.sum(K.square(true_box_angle - pred_box_angle), axis=-1), axis=-1
    )
...
def calc_loss_class(true_box_class, pred_box_class, entropy):
    ...
    if entropy == "notbinary":
        loss_class = K.mean(
            K.sparse_categorical_crossentropy(true_box_class, pred_box_class), axis=-1
        )
...
def volume_yolo_loss(
    categories, grid_h, grid_w, grid_d, nboxes, box_vector, entropy
):
    def loss(y_true, y_pred):
        ...
        (
            ...
            true_box_whd,
            true_box_conf,
        ) = extract_ground_event_volume_truth(
            y_true, categories, grid_h, grid_w, grid_d, nboxes, box_vector
        )
        (
            pred_box_class,
            ...
            pred_box_whd,
            pred_box_conf,
        ) = extract_ground_event_volume_pred(
            y_pred,
            categories,
            grid_h,
            grid_w,
...
def static_yolo_loss(categories, grid_h, grid_w, nboxes, box_vector, entropy):
    def loss(y_true, y_pred):
        ...
        (
            ...
            true_box_wh,
            true_box_conf,
        ) = extract_ground_cell_truth(
            y_true, categories, grid_h, grid_w, nboxes, box_vector
        )
        (
            pred_box_class,
            pred_box_xy,
            pred_box_wh,
            pred_box_conf,
        ) = extract_ground_cell_pred(
            y_pred, categories, grid_h, grid_w, cell_grid, nboxes, box_vector
        )

        loss_xywh = calc_loss_xywh(
...
def static_yolo_loss_segfree(
    ...
    def loss(y_true, y_pred):
        ...
        cell_grid = get_cell_grid(grid_h, grid_w, nboxes)
        true_box_class, true_box_xy = extract_ground_cell_truth_segfree(
            y_true, categories, grid_h, grid_w, nboxes, box_vector
        )
        pred_box_class, pred_box_xy = extract_ground_cell_pred_segfree(
            y_pred, categories, grid_h, grid_w, cell_grid, nboxes, box_vector
        )
