...
class ScoreModels:
    ...
    def model_scorer(self):
        ...
        for i in range(len(self.listtime_gt)):
            ...
            self.location_gt.append(
                [self.listtime_gt[i], self.listy_gt[i], self.listx_gt[i]]
            )
        ...
        for csv_pred in self.predictions:
            ...
            for i in range(len(self.listtime_pred)):
                ...
                if float(self.listscore_pred[i]) >= float(self.thresholdscore):
                    self.location_pred.append(
                        [
                            int(self.listtime_pred[i]),
                            int(self.listy_pred[i]),
                            int(self.listx_pred[i]),
                        ]
                    )
    ...
    def TruePositives(self):
        ...
        for i in range(len(self.location_pred)):
            ...
            spacedistance, timedistance = TimedDistance(
                return_index, self.location_gt[closestpoint[1]]
            )

            if (
                spacedistance < self.thresholdspace
                and timedistance < self.thresholdtime
            ):
                tp = tp + 1
            else:
                fp = fp + 1
    ...
    def FalseNegatives(self):
        ...
        for i in range(len(self.listtime_gt)):
            ...
            return_index = (
                int(self.listtime_gt[i]),
                int(self.listy_gt[i]),
                int(self.listx_gt[i]),
            )
            closestpoint = tree.query(return_index)
            spacedistance, timedistance = TimedDistance(
                return_index, self.location_pred[closestpoint[1]]
            )

            if (
                spacedistance > self.thresholdspace
                or timedistance > self.thresholdtime
            ):
                fn = fn + 1
